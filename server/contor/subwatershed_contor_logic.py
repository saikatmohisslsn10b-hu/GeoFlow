...
def generate_subwatershed_contour_map(cn_raster_path, main_shapefile, sub_shapefile, output_png, title):
    ...
    cn_masked = np.where(cn_matrix > 0, cn_matrix, np.nan)
    cn_filled = np.where(np.isnan(cn_masked), np.nanmean(cn_masked), cn_masked)
    cn_smoothed = gaussian_filter(cn_filled, sigma=20.0)
    cn_smoothed = np.where(cn_matrix > 0, cn_smoothed, np.nan)

    sub_gdf = gpd.read_file(sub_shapefile).to_crs("EPSG:4326")
    sub_gdf['sub_x'] = sub_gdf.geometry.centroid.x
    sub_gdf = sub_gdf.sort_values(by='sub_x').reset_index(drop=True)

    if main_shapefile:
        main_gdf = gpd.read_file(main_shapefile).to_crs("EPSG:4326")
    ...
    headers = {'User-Agent': 'EasyGIS_Academic_App/1.0 (dipan.lahiri@student.edu) Python-urllib'}
    subdomains = ['a', 'b', 'c']
    ...
    contour_levels = np.arange(30, 105, 5) 
    extent_raster = [lon_min, lon_max, lat_min, lat_max]

    contour_fill = ax.contourf(cn_smoothed, levels=contour_levels, cmap='Spectral_r', 
                               extend='max', extent=extent_raster, origin='upper', zorder=2, alpha=0.85)
    contour_lines = ax.contour(cn_smoothed, levels=contour_levels, colors='black', 
                               linewidths=0.5, alpha=0.6, extent=extent_raster, origin='upper', zorder=3)

    sub_gdf.plot(ax=ax, edgecolor="white", linewidth=0.8, facecolor="none", zorder=4)
    ...
    for idx, row in sub_gdf.iterrows():
        ...
        ax.text(centroid.x, centroid.y, s_label, 
                ...
                zorder=5, fontname="Times New Roman")

    ax.set_xlim(-99.80, -96.70)
    ax.set_ylim(28.40, 30.60)
    ax.grid(True, which='both', color='gray', linestyle='--', linewidth=0.5, alpha=0.5, zorder=6)
    ...
    ax.add_patch(mpatches.Rectangle((x_base, y_base), scale_25km, bar_height, edgecolor='black', facecolor='black', zorder=7))
    ax.add_patch(mpatches.Rectangle((x_base + scale_25km, y_base), scale_25km, bar_height, edgecolor='black', facecolor='white', zorder=7))
    label_y = y_base + bar_height * 1.3
    ax.text(x_base, label_y, "0", ha='center', va='bottom', color='black', weight='bold', fontsize=9, zorder=8, fontname="Times New Roman")
    ax.text(x_base + scale_25km, label_y, "25", ha='center', va='bottom', color='black', weight='bold', fontsize=9, zorder=8, fontname="Times New Roman")
    ax.text(x_base + scale_50km, label_y, "50 km", ha='center', va='bottom', color='black', weight='bold', fontsize=9, zorder=8, fontname="Times New Roman")

    x_arrow, y_arrow = -96.95, 30.45
    ax.annotate('', xy=(x_arrow, y_arrow), xytext=(x_arrow, y_arrow - 0.08), arrowprops=dict(arrowstyle="->", lw=2, color='black'), zorder=8)
    ax.text(x_arrow, y_arrow - 0.11, "N", ha='center', va='center', color='black', fontname="Times New Roman", weight='bold', fontsize=14, zorder=8)

    cbar = plt.colorbar(contour_fill, shrink=0.7, pad=0.08, ticks=np.arange(30, 105, 5))
    cbar.set_label('Curve Number (CN)', fontsize=14, weight='bold', fontname="Times New Roman")
    cbar.ax.tick_params(labelsize=10)
    for tick in cbar.ax.get_yticklabels():
        tick.set_fontname("Times New Roman")

    final_title = str(title).strip() if title else "Guadalupe River Basin (Subwatershed) Contour Map"
